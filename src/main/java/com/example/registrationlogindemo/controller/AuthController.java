...
public class AuthController {
    ...
    public AuthController(@Autowired ProductServiceMySQL productServiceMySQL, @Autowired CartItemServiceImpl itemService, @Autowired CartItemServiceImpl cartItemService, @Autowired OrderServiceImpl orderServiceImpl, @Autowired UserServiceImpl userService) {
...
//------------------------------FOR BOTH ADMIN AND USER ROLE---------------------------------------------------------//
    ...
    public String indexUser(Model model, Authentication authentication){
        ...
        System.out.println("call index get mapping, " + userService.findByEmail(authentication.getName()).getName());
        return "indexUser";
    }
...
//        System.out.println("user after log in is: " + userService.findByEmail(authentication.getName()).getId());
    ...
    public String productList(@Param("keyword") String keyword, Model model, Authentication authentication){
        model.addAttribute("userName", userService.findByEmail(authentication.getName()).getName());
        model.addAttribute("productList", productServiceMySQL.listProductsAccordingToSearchKeyword(keyword));
            ...
            for (Order o : ordersOfThisUser) {
                if (o.getStatus().equals("pending")) {
                    ...
                    for (CartItem c : o.getCartItemsList()) {
                        //Case 1: user has an existing cart item with this product => change quantity, change pay amount
                        if (c.getProductID() == cartItemDto.getProductID()) {
                            ...
                            payAmount += cartItemFound.getProductQuantity() * productServiceMySQL.findById(cartItemFound.getProductID()).getPrice();
...
//        }
    ...
    public String PostMappingCheckOut(Principal principal){
        ...
        //this.orderServiceImpl.findBUserId(user.getId()).setStatus("completed"); //have to use update method, not setter
